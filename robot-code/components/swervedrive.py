...
class SwerveDrive:
    ...
    xy_multiplier = 1
    debugging = False # Turn to true to run it in verbose mode.
    NavxYawZero = 0
    NavxYawZero = 0
    ...
    def _calculate_vectors(self):
        ...
        # Convert to field centric vector with
        navxYaw = (self.navx.getYaw() - self.NavxYawZero) / 360 * 2 * math.pi
        print(self.navx.getYaw(), self.NavxYawZero, navxYaw)
        temp = self._requested_vectors['fwd'] * math.cos(navxYaw) + self._requested_vectors['strafe'] * math.sin(navxYaw)
        self._requested_vectors['strafe'] = -self._requested_vectors['fwd'] * math.sin(navxYaw) + self._requested_vectors['strafe'] * math.cos(navxYaw)
        self._requested_vectors['fwd'] = temp
        # Where θ is the gyro angle, measured clockwise from the zero position (zero normally being set to straight downfield):
        ...
        frontRight_speed = math.hypot(B, C) * 0.25
        frontRight_angle = math.degrees(math.atan2(B, C))

        frontLeft_speed = math.hypot(B, D) * 0.25
        frontLeft_angle = math.degrees(math.atan2(B, D))

        rearLeft_speed = math.hypot(A, D) * 0.25
        rearLeft_angle = math.degrees(math.atan2(A, D))

        rearRight_speed = math.hypot(A, C) * 0.25
